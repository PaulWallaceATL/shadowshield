...
const SuperAdminAPIManagementSection = forwardRef<APIManagementHandle, {}>((props, ref) => {
                ...
                <div className="p-4 sm:p-5">
                  <div className="flex justify-between items-center mb-3 flex-wrap sm:flex-nowrap gap-2">
                    <div className="flex items-center">
                      <img src={provider.logo} alt={provider.name} className="h-6 w-6 sm:h-8 sm:w-8 mr-2" />
                      <h3 className={`text-lg sm:text-xl font-bold ${provider.textColor}`}>{provider.name}</h3>
                    </div>
                    <span className="px-2 sm:px-3 py-1 text-xs sm:text-sm font-bold rounded-full bg-green-600 text-white shadow-sm">
                      {apiKey.stats.successRate.toFixed(1)}% Success
                    </span>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className={`${provider.lightColor} p-3 rounded-lg shadow-sm`}>
                      <p className={`text-xs font-medium ${provider.textColor} mb-1`}>Queries (24h)</p>
                        ...
                        <div className="flex items-center text-xs mt-1">
                          {apiKey.stats.trend === 'up' ? (
                            <>
                              ...
                              <span className="text-green-500 font-medium">+{apiKey.stats.trendPercentage.toFixed(1)}%</span>
                            </>
                          ) : apiKey.stats.trend === 'down' ? (
                            <>
                              <ArrowDownIcon className="h-3 w-3 text-red-500 mr-1" />
                              <span className="text-red-500 font-medium">{apiKey.stats.trendPercentage.toFixed(1)}%</span>
                            </>
                          ) : null}
                        </div>
                      )}
                    ...
                    <div className={`${provider.lightColor} p-3 rounded-lg shadow-sm`}>
                      <p className={`text-xs font-medium ${provider.textColor} mb-1`}>Avg. Latency</p>
                      <p className={`text-lg font-bold ${provider.textColor}`}>{apiKey.stats.averageLatency.toFixed(0)}ms</p>
                    </div>
                  </div>
                  
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    <div className={`${provider.lightColor} p-2 rounded-lg text-center shadow-sm`}>
                      <p className={`text-[10px] font-medium ${provider.textColor}`}>Total</p>
                      <p className={`text-sm font-semibold ${provider.textColor}`}>{apiKey.stats.totalQueries.toLocaleString()}</p>
                    </div>
                    
                    <div className={`${provider.lightColor} p-2 rounded-lg text-center shadow-sm`}>
                      <p className={`text-[10px] font-medium ${provider.textColor}`}>DLP Violations</p>
                      <p className={`text-sm font-semibold ${provider.textColor}`}>{apiKey.stats.dlpFlaggedQueries.toLocaleString()}</p>
                    </div>
                    
                    <div className={`${provider.lightColor} p-2 rounded-lg text-center shadow-sm`}>
                      <p className={`text-[10px] font-medium ${provider.textColor}`}>Users</p>
...
SuperAdminAPIManagementSection.displayName = 'SuperAdminAPIManagementSection';
export default SuperAdminAPIManagementSection; 